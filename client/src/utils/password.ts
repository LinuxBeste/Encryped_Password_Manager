...
export function scorePassword(password: string): PasswordScore {
  ...
  const colors = ['var(--accent-red)', 'var(--accent-amber)', '#e3b341', 'var(--accent-green)', 'var(--accent-green)'];
  return {
    score: result.score,
...
export function generatePassword(
  ...
  if (options.excludeAmbiguous) {
    validChars = [...chars].filter((c) => !AMBIGUOUS.includes(c)).join('');
  }
  if (!validChars) validChars = chars;
...
export function generatePassphrase(options: {
  ...
  return {
    password: passphrase,
    score: scorePassword(passphrase),
  };
}

...
describe('DB — users CRUD', () => {
  ...
  it('deletes user cascade-deletes related data', () => {
    ...
    ).run(vid, uid, 'v', emptyBuf, emptyBuf, emptyBuf, 1);
    db.prepare(
      'INSERT INTO entries (id, vault_id, user_id, title_encrypted, body_encrypted, iv, auth_tag, type, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
    ).run(
      eid,
      vid,
      uid,
...
describe('DB — vaults CRUD', () => {
  it('inserts vault with default values', () => {
    ...
    db.prepare(
      'INSERT INTO vaults (id, user_id, name, encrypted_blob, iv, auth_tag, version, updated_at) VALUES (?, ?, ?, ?, ?, ?, 1, ?)',
    ).run(vid, uid, 'default', Buffer.alloc(0), Buffer.alloc(0), Buffer.alloc(0), 1);
...
describe('DB — entries CRUD', () => {
  function seed() {
    ...
    db.prepare(
      'INSERT INTO vaults (id, user_id, name, encrypted_blob, iv, auth_tag, version, updated_at) VALUES (?, ?, ?, ?, ?, ?, 1, ?)',
    ).run(vid, uid, 'v', Buffer.alloc(0), Buffer.alloc(0), Buffer.alloc(0), 1);
    return { db, uid, vid };
  }
  ...
  it('inserts entry with all fields', () => {
    ...
    const eid = uuidv4();
    db.prepare(
      `INSERT INTO entries (id, vault_id, user_id, title_encrypted, body_encrypted, iv, auth_tag, type, tags_encrypted, favorite, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    ).run(
      eid,
      vid,
  ...
  it('allows null folder_id', () => {
    const { db, uid, vid } = seed();
    db.prepare(
      `INSERT INTO entries (id, vault_id, user_id, title_encrypted, body_encrypted, iv, auth_tag, type, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    ).run(
      uuidv4(),
      vid,
  ...
  it('filters out soft-deleted with WHERE clause', () => {
    ...
    db.prepare(
      `INSERT INTO entries (id, vault_id, user_id, title_encrypted, body_encrypted, iv, auth_tag, type, created_at, updated_at, deleted_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    ).run(
      uuidv4(),
      vid,
...
describe('DB — folders CRUD', () => {
  it('inserts folders with sort_order default', () => {
    ...
    ).run(fid, vid, uid, Buffer.from('fn'), 0, 1);
  ...
  it('supports parent_id hierarchy', () => {
    ...
    const p = uuidv4(),
      c = uuidv4();
    db.prepare(
      'INSERT INTO folders (id, vault_id, user_id, name_encrypted, parent_id, sort_order, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
    ).run(p, vid, uid, Buffer.from('p'), null, 0, 1);
    db.prepare(
      'INSERT INTO folders (id, vault_id, user_id, name_encrypted, parent_id, sort_order, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
    ).run(c, vid, uid, Buffer.from('c'), p, 1, 1);
...
describe('DB — refresh_tokens', () => {
  it('inserts and queries by hash', () => {
    ...
    ).run(rid, uid, 'abc123hash', 9999999999999, 1);
    const r = db
      .prepare('SELECT * FROM refresh_tokens WHERE token_hash = ?')
      .get('abc123hash') as any;
  ...
  it('tracks revoked status', () => {
    ...
    db.prepare(
      'INSERT INTO refresh_tokens (id, user_id, token_hash, expires_at, revoked, created_at) VALUES (?, ?, ?, ?, 0, ?)',
    ).run(rid, uid, 'hash2', 9999999999999, 1);
...
describe('DB — settings', () => {
  it('inserts and updates settings', () => {
    ...
    ).run(uid, 'theme', 'dark', 1);
    db.prepare(
      'INSERT OR REPLACE INTO settings (user_id, key, value, updated_at) VALUES (?, ?, ?, ?)',
    ).run(uid, 'theme', 'light', 2);
    const s = db
      .prepare('SELECT value FROM settings WHERE user_id = ? AND key = ?')
      .get(uid, 'theme') as any;
  ...
  it('stores multiple settings per user', () => {
    ...
    db.prepare('INSERT INTO settings (user_id, key, value, updated_at) VALUES (?, ?, ?, ?)').run(
      uid,
      'k1',
      'v1',
      1,
...
describe('DB — constraints & edge cases', () => {
  it('rejects INSERT without required fields (email)', () => {
    const db = freshDb();
    expect(() => {
      db.prepare('INSERT INTO users (id, argon2_hash, created_at) VALUES (?, ?, ?)').run(
        uuidv4(),
        'h',
        1,
      );
  ...
  it('rejects duplicate vault name per user (no unique enforced — just check insert works)', () => {
    ...
    db.prepare(
      'INSERT INTO vaults (id, user_id, name, encrypted_blob, iv, auth_tag, version, updated_at) VALUES (?, ?, ?, ?, ?, ?, 1, ?)',
    ).run(vid1, uid, 'same', Buffer.alloc(0), Buffer.alloc(0), Buffer.alloc(0), 1);
    expect(() => {
      db.prepare(
        'INSERT INTO vaults (id, user_id, name, encrypted_blob, iv, auth_tag, version, updated_at) VALUES (?, ?, ?, ?, ?, ?, 1, ?)',
      ).run(vid2, uid, 'same', Buffer.alloc(0), Buffer.alloc(0), Buffer.alloc(0), 1);
    }).not.toThrow();
  });
  ...
  it('cleans up all tables via CASCADE on user delete', () => {
    ...
    db.prepare('DELETE FROM users WHERE id = ?').run(uid);
    expect(db.prepare('SELECT COUNT(*) as c FROM vaults WHERE user_id = ?').get(uid)).toEqual({
      c: 0,
    });
    expect(db.prepare('SELECT COUNT(*) as c FROM folders WHERE user_id = ?').get(uid)).toEqual({
      c: 0,
    });
    expect(db.prepare('SELECT COUNT(*) as c FROM entries WHERE user_id = ?').get(uid)).toEqual({
      c: 0,
    });
    expect(
      db.prepare('SELECT COUNT(*) as c FROM refresh_tokens WHERE user_id = ?').get(uid),
    ).toEqual({ c: 0 });
    expect(db.prepare('SELECT COUNT(*) as c FROM settings WHERE user_id = ?').get(uid)).toEqual({
      c: 0,
    });
  });
});
